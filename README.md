...
网页名称是“黄金保守马丁格尔策略”。API key、secret 和部分交易所需要的 passphrase 只发送到后端内存，不会写入文件。建议设置控制台登录密码：

```bash
export APP_PASSWORD="换成一个足够长的管理密码"
export APP_SECRET="换成一段随机字符串"
```

更安全的个人使用方式是把交易所密钥放在服务器环境变量里，而不是在网页里填写：
...
- 程序优先支持从环境变量读取 API key，不打印、不保存 key。
- 网页版从表单读取 API key，只保存在运行中的后端内存；服务重启后需要重新绑定。
- 公网部署前必须设置 `APP_PASSWORD`，并且交易所 API 必须关闭提现权限。
- 公网环境如果没有设置 `APP_PASSWORD`，控制台会自动锁住；本地 `127.0.0.1` 调试可以不设置。
